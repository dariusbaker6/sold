...
with st.sidebar:
    ...
    refresh_seconds = st.slider("Auto refresh seconds (hard reload)", 5, 60, 15, 5)
...
with tab_radar:
    ...
    recent_pairs = fetch_table(
        "pairs",
        select="pair_address,token_address:base_token,base_token_name,base_token_symbol,pair_created_at,snapshot_ts",
...
# ============================= Hard browser reload =============================
_reload_ms = int(max(5, int(refresh_seconds)) * 1000)
st.markdown(f"""
<script>
  try {{
    if (window.__hardReloadTimer) clearTimeout(window.__hardReloadTimer);
    window.__hardReloadTimer = setTimeout(function() {{
